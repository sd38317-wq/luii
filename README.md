...
네이버플레이스로 들어온 예약을 관리자가 간단히 등록해두면, **문자 3통을 자동으로 발송**해주는 내부 관리 도구입니다.
...
2. 서버가 **1분마다** 예약 목록을 확인해서, 위 3가지 시점에 맞춰 아직 안 보낸 문자를 자동 발송합니다.
...
- **카카오 알림톡 전환**: 카카오톡 채널(`바른프라이빗키즈룸(괴정점)`, 검색용 URL `http://pf.kakao.com/_BExhxnX`) 개설 완료, 비즈니스 심사(사업자 인증) 진행 중 (영업일 3~5일 소요). 심사 완료 후 알리고에 채널 연동 → 문구 3종 템플릿 승인까지 받아야 실제 전환 가능.
...
src/lib/sms.ts                      # 알리고 SMS 발송 + 문자 3종 문구 생성
src/lib/reminder.ts                 # 3가지 발송 시점 판정 및 발송 로직 (1분마다 cron 실행)
