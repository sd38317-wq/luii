...
- **문자 4종은 각각 켜고 끌 수 있습니다** — `/admin` 설정 맨 위 "문자 발송 켜기/끄기"에서 체크를 해제하면 그 문자는 발송되지 않습니다 (다른 가게에 팔 때 "리뷰 문자는 필요 없어요" 같은 요청에 코드 수정 없이 대응하기 위한 기능).
...
- 화면에 표시되는 앱 이름(로그인 화면, 브라우저 탭, 홈 화면 추가 시 이름)도 `/admin` 설정의 카페명을 그대로 따라갑니다 — 코드에 가게 이름이 박혀있지 않아서, 같은 코드를 다른 가게에 배포해도 설정만 바꾸면 됩니다.
- **발송 이력·이번 달 현황**: `/admin`의 "발송 이력 · 이번 달 현황"에서 문자가 언제 누구에게 나갔는지(실패했으면 왜 실패했는지)와 이번 달 예약/발송 성공/실패 건수를 확인할 수 있습니다. `fly logs`를 안 봐도 폰에서 바로 확인돼요. 기록은 90일 보관 후 자동 삭제됩니다.
...
1. 네이버 예약 알림이 오면, 관리자가 `/admin` 페이지에서 고객명/연락처/이용시간을 등록합니다. 예약창을 캡처해서 올리면 사진에서 자동으로 읽어 채워줍니다 ("📷 예약창 캡처로 자동 입력", Claude API 사용). **사진을 한 번에 여러 장 선택하면** 각 사진을 순서대로 읽어서 바로 예약 목록에 등록까지 해줍니다(여러 명 예약을 한 번에 처리할 때 유용). 사진 한 장만 선택하면 기존처럼 입력칸에 채워주고 확인 후 등록 버튼을 눌러야 합니다.
   - **중복 등록 방지**: 같은 전화번호로 같은 시각의 예약이 이미 있으면 등록을 막습니다(사진을 실수로 두 번 올려도 안전).
